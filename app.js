    ...
    conn.on('data', function(message) {
        ...
        if(rateLimit[conn.id] < 1 && JSON.parse(message).type != 'delete' && JSON.parse(message).type != 'typing' && JSON.parse(message).type != 'ping') {
            ...
            try {
                ...
                if(data.type == 'pm') log('message', chalk.underline(clients[conn.id].un) + ' - ' + chalk.underline(data.extra) + ': ' + data.message);
    ...
    switch(data.type) {
        case 'pm':
            if(data.extra != data.user && utils.checkUser(clients, data.extra)) {
                ...
                data.subtxt = 'Özel Mesaj: ' + data.extra;
                utils.sendBack(clients, data, user);
            } else {
                data.type = 'light';
                data.subtxt = null;
                data.message = utils.checkUser(clients, data.extra) ? 'Kendine özel mesaj gönderemezsin.' : 'Kullanıcı bulunamadı.';
                utils.sendBack(clients, data, user);
            }
            break;
            ...
            if(user.role > 0) {
                if(data.type == 'global') {
                    if(user.role == 3) {
                        ...
                        data.subtxt = null;
                        data.message = 'Bunu yapmaya izniniz yok!';
                    ...
                    if(data.message != data.user) {
                            ...
                            switch(data.type) {
                                case 'ban':
                                    ...
                                    if(!isNaN(time) && time > 0) {
                                        if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                            ...
                                            data.message = data.user + time + ' dakika için ' + data.message + ' sunucudan yasaklandı. ' ;
                                            return utils.sendToAll(clients, data);
                                        } else {
                                            data.message = 'Bunu yapmaya izniniz yok!';
                                            return utils.sendBack(clients, data, user);
                                        ...
                                        data.type = 'light';
                                        data.message = '/ban [kullanici_adi] [dakika] komutunu kullanın.';
                                ...
                                case 'role':
                                    if(data.extra > -1 && data.extra < 4) {
                                        ...
                                        data.type = 'light';
                                        data.message = '/role [kullanici_adi] [0-3] komutunu kullanın.';
                                        return utils.sendToOne(clients, users, data, data.user, 'message')
                                    }
                                    break;
                                ...
                                case 'kick':
                                    if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                        data.extra = data.message;
                                        data.message = data.user + ' bu sunucudan ' + data.message + ' yasaklandı.';
                                    } else {
                                        data.message = 'Bunu yapmaya izniniz yok!';
                                        return utils.sendBack(clients, data, user);
                                    }
                        ...
                        data.message = 'Bunu yapamazsınız!';
                        utils.sendBack(clients, data, user);
                    }
                }
            } else {
...
function onError(error) {
    ...
    switch(error.code) {
        case 'EACCES':
            console.error(bind + ' yüksek ayrıcalıklar gerektirir.');
            process.exit(1);
            break;
